...
function updateCarOffset(car, carSegment, playerSegment, playerW) {
	...
	let i, j, dir, segment, otherCar, otherCarW, lookahead = 20, carW = car.gameObject.w * GAMEOBJECTS.SCALE;
	...
	for(i = 1 ; i < lookahead ; i++) {
		...
		if ((segment === playerSegment) && (car.speed > speed) && (isOverlapping(playerX, playerW, car.offset, carW, 1.2))) {
			if (playerX > 0.5)
				dir = -1;
			else if (playerX < -0.5)
				dir = 1;
		...
		for(j = 0 ; j < segment.carsArray.length ; j++) {
			otherCar  = segment.carsArray[j];
			otherCarW = otherCar.gameObject.w * GAMEOBJECTS.SCALE;
			if ((car.speed > otherCar.speed) && isOverlapping(car.offset, carW, otherCar.offset, otherCarW, 1.2)) {
...
function resetRoad() {
	...
	roadSegmentsArray[findSegment(playerZ).index + 2].color = COLORS.START;
	roadSegmentsArray[findSegment(playerZ).index + 3].color = COLORS.START;
